        ...
        <div className="w-full rounded-2xl bg-secondary p-3 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
                { walletProvider.map((provider) => (
                    <button onClick={() => setPick(provider.id)} key={provider.id} className={`flex gap-3 items-center text-h5 rounded-2xl p-3 transition-all duration-300 ease-in-out ${provider.id === pick ? 'bg-primary scale-105' : 'bg-transparent'}`}>
                        <div className="relative overflow-hidden w-15 h-15 rounded-2xl">
